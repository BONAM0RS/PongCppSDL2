      ...
      m_maxFrameRate { 16 } // 60 FPS
{
    ...
    {
        std::cout << " SDL_image system is ready to go" << std::endl;
    }
    ...
    int status = Mix_Init(mixFlag);
    if ((status & mixFlag) != mixFlag)
    {
        std::cout << " SDL2_mixer format not available:" << SDL_GetError() << std::endl;
    }
    else
    {
        std::cout << " SDL2_mixer system ready to go! \n" << std::endl;
    }
    */
...
void createObject(std::string name, LayerPriority priority)
{
