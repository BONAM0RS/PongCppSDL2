...
void ObjectManager::registerObject(const std::string &name, LayerPriority priority)
{
    ...
    if (m_objects[name] != nullptr) { std::cout << "\t The Object '" << name << "' has been registered \n" << std::endl; }
    else                            { std::cout << "\t Error! The Object '" << name << "' has not been registered!" << std::endl; }
}
...
SDL_Renderer* ObjectManager::getRenderer() const 
{ 
    return m_renderer; 
}
